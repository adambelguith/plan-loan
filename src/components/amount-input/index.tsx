...
export const AmountInput = ({ amount, setAmount, minAmount, maxAmount }: AmountInputProps) => {
    const handelChange = (e: any) => {
        setAmount(e.target.value)
    }
    // const stramount: string = amount.toString()
    // const addCommas = (stramount) => stramount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
    // const removeNonNumeric = (stramount) => stramount.toString().replace(/[^0-9]/g, "")
    ...
    // let thouo = thousandSeparator(amount, "./")
    console.log(typeof amount)

    return (
        <div className="glob-amount">
